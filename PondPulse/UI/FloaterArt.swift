...
nonisolated func drawFloaterSkin(_ ctx: inout GraphicsContext, skinId: String, rect: CGRect, palette: PondPalette, color: DuckColor?) {
    ...
    // The theme pairs (FloaterArtThemed), the special five
    // (FloaterArtSpecial) and the far end of every ladder (FloaterArtLegends).
    case "toucan": drawToucan(&ctx, rect, palette, color)
    case "treefrog": drawTreeFrog(&ctx, rect, palette, color)
    case "mandarin": drawMandarin(&ctx, rect, palette, color)
    case "goldfish": drawGoldfish(&ctx, rect, palette, color)
    case "swallow": drawSwallow(&ctx, rect, palette, color)
    case "opalkoi": drawOpalKoi(&ctx, rect, palette, color)
    case "pearlswan": drawPearlSwan(&ctx, rect, palette, color)
    case "emberdrake": drawEmberDrake(&ctx, rect, palette, color)
    case "cindermoth": drawCinderMoth(&ctx, rect, palette, color)
    case "blossomkoi": drawBlossomKoi(&ctx, rect, palette, color)
    case "neontetra": drawNeonTetra(&ctx, rect, palette, color)
    case "glowjelly": drawGlowJelly(&ctx, rect, palette, color)
    case "wooduck": drawWoodDuck(&ctx, rect, palette, color)
    case "squirrel": drawSquirrel(&ctx, rect, palette, color)
    case "snowgoose": drawSnowGoose(&ctx, rect, palette, color)
    case "icefish": drawIceFish(&ctx, rect, palette, color)
    case "clownfish": drawClownfish(&ctx, rect, palette, color)
    case "starfish": drawStarfish(&ctx, rect, palette, color)
    case "cometduck": drawCometDuck(&ctx, rect, palette, color)
    case "moonjelly": drawMoonJelly(&ctx, rect, palette, color)
    case "gummyduck": drawGummyDuck(&ctx, rect, palette, color)
    case "bubblegum": drawBubblegumFish(&ctx, rect, palette, color)
    case "royalswan": drawRoyalSwan(&ctx, rect, palette, color)
    case "peacock": drawPeacock(&ctx, rect, palette, color)
    case "starwhale": drawStarWhale(&ctx, rect, palette, color)
    case "kitsune": drawKitsune(&ctx, rect, palette, color)
    case "griffin": drawGriffin(&ctx, rect, palette, color)
    case "seadragon": drawSeaDragon(&ctx, rect, palette, color)
    case "moonrabbit": drawMoonRabbit(&ctx, rect, palette, color)
    case "goldenturtle": drawGoldenTurtle(&ctx, rect, palette, color)
    case "firefly": drawFirefly(&ctx, rect, palette, color)
    case "owl": drawOwl(&ctx, rect, palette, color)
    case "kraken": drawKraken(&ctx, rect, palette, color)
    case "anglerfish": drawAnglerfish(&ctx, rect, palette, color)
    case "manta": drawManta(&ctx, rect, palette, color)
    case "raven": drawRaven(&ctx, rect, palette, color)
    case "lionfish": drawLionfish(&ctx, rect, palette, color)
...
nonisolated func drawPadStyle(_ ctx: inout GraphicsContext, padId: String, rect: CGRect, palette: PondPalette, ring: Color?) {
    ...
    // The far end of every ladder (FloaterArtLegends).
    case "crownlily": drawCrownLilyPad(&ctx, rect)
    case "ember": drawEmberPad(&ctx, rect)
    case "frost": drawFrostPad(&ctx, rect)
    case "pearl": drawPearlPad(&ctx, rect)
    case "obsidian": drawObsidianPad(&ctx, rect)
    case "origami": drawOrigamiPad(&ctx, rect)
    case "rune": drawRunePad(&ctx, rect)
    case "prism": drawPrismPad(&ctx, rect)
